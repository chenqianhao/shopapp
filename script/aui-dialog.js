    ...
    auiDialog.prototype = {
        ...
        create: function(params,callback) {
            ...
            if(params.input){
                ...
                var msgHtml = '<div class="aui-dialog-body"><input type="password" id="'+params.id+'" placeholder="'+params.text+'" style="font-size:0.8rem;"></div>';
            }else{
            ...
            if(dialogButtons && dialogButtons.length > 0){
                for(var ii = 0; ii < dialogButtons.length; ii++){
                    dialogButtons[ii].onclick = function(){
                        if(callback){
                            if(params.input){
                            	//var text_value=document.getElementById(params.id).value;
                            	//alert(text_value);
                                callback({
                                    buttonIndex: parseInt(this.getAttribute("button-index"))+1,
                                    text: document.getElementById(params.id).value
